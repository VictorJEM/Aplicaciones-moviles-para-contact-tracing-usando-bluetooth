...
public class CryptoModule {
	...
	protected List<EphId> createEphIds(byte[] SK, boolean shuffle) {
		//SK -> "Secret Key"
		try {
			String key_32bytes = new String(Hex.encodeHex(SK));
			Log.i("info","Llave aleatoria de 32 bytes: "+key_32bytes);
			Mac mac = Mac.getInstance("HmacSHA256");
			mac.init(new SecretKeySpec(SK, "HmacSHA256"));
			mac.update(BROADCAST_KEY);
			byte[] prf = mac.doFinal();

			//generate EphIDs of 16 bytes
			...
			//muestra las 96 llaves de 16 bytes creadas para un día
			/*int i = 1;
			for(EphId eph: ephIds){
				Log.i("info","createEphIds - "+i+". "+Hex.encodeHexString(eph.getData()));
				i++;
			}*/
	...
	public void checkContacts(byte[] sk, long onsetDate, long bucketTime, GetContactsCallback contactCallback, MatchCallback matchCallback) {
	...
	public ExposeeRequest getSecretKeyForPublishing(DayDate date, ExposeeAuthMethod exposeeAuthMethod, HashMap<String,String> location) {
		SKList skList = getSKList();
		...
		for (Pair<DayDate, byte[]> daySKPair : skList) {
			if (daySKPair.first.equals(date)) {
				return new ExposeeRequest(
						...
						jsonAuth,
						location.get("provincia"),
						location.get("canton"),
						location.get("parroquia"));
			}
		}
